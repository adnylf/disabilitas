  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80 dark:bg-gray-900/95 dark:supports-[backdrop-filter]:bg-gray-900/80">
      <nav className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          ...
          <div className="flex items-center gap-8">
            <Link href="/" className="flex items-center gap-2 transition-transform hover:scale-105">
              ...
              <span className="text-xl font-bold text-gray-900 dark:text-white">EduAccess</span>
            </Link>
            ...
            <div className="hidden md:flex items-center gap-6">
              ...
              <div className="relative">
                ...
                >
                  Kursus
                  <ChevronDown className={`h-4 w-4 transition-transform duration-300 ${coursesDropdownOpen ? 'rotate-180' : ''} group-hover:translate-y-0.5`} />
                </button>
        ...
        {/* Mobile Menu */}
        {mobileMenuOpen && (
          <div className="md:hidden py-4 border-t border-gray-200 dark:border-gray-700 animate-in fade-in-0 slide-in-from-top-2 duration-300">
            <div className="flex flex-col gap-4">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-gray-900 dark:text-white">Kursus Gratis</h4>
                      <p className="text-xs text-gray-600 dark:text-gray-400">Mulai belajar tanpa biaya</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-2 ml-2">
                    {freeCourses.map((course, index) => (
                      <Link
                        key={index}
                        href={`/courses/free/${course.toLowerCase().replace(/\s+/g, '-')}`}
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-gray-900 dark:text-white">Kursus Berbayar</h4>
                      <p className="text-xs text-gray-600 dark:text-gray-400">Akses konten premium lengkap</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-2 ml-2">
                    {paidCourses.map((course, index) => (
                      <Link
                        key={index}
                        href={`/courses/paid/${course.toLowerCase().replace(/\s+/g, '-')}`}
              ...
              <Link href="/instructor/apply">
                <DrawOutlineButton className="w-full text-[#005EB8]" onClick={handleMobileLinkClick}>
                  Jadi Instruktur
                </DrawOutlineButton>
              </Link>
